...
const EDIT_URL_BASE = process.env.EDIT_URL_BASE ?? 'https://github.com/constructorfabric/gears-rust/edit/main/docs/web-docs/'
...
function syncFile(relPath) {
  ...
  const editUrl = EDIT_URL_BASE + relPath
  if (content.startsWith('---')) {
    const closeIdx = content.indexOf('\n---\n', 3)
    if (closeIdx !== -1 && !/^editUrl:/m.test(content.slice(0, closeIdx))) {
      const insertPos = closeIdx + 1 // after the leading newline, before the closing ---
      content = content.slice(0, insertPos) + `editUrl: ${editUrl}\n` + content.slice(insertPos)
    }
  } else {
    content = `---\neditUrl: ${editUrl}\n---\n\n${content}`
  }
